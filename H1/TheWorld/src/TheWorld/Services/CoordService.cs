...
namespace TheWorld.Services
{
    ...
    {
        private readonly ILogger<CoordService> coordLogger;

        public CoordService(ILogger<CoordService> coordLogger)
        {
            this.coordLogger = coordLogger;
        }

        public async Task<CoordServiceResult> Lookup(string location)
        {
            var result = new CoordServiceResult() { Success = false, Message = "Undetermined failure while looking up coordinates" };
